    ...
    return Scaffold(
        ...
        body: Column(
          children: [
            ...
            TextButton(
                onPressed: (() async {
                  var password = await DatabaseService(user: user)
                      .getClubPassword(widget.clubName);
                  if (controller.text == password) {
                    setState(() {
                      error = "Successfully Logged Into The Club!";
                    });
                    DatabaseService(user: user)
                        .updateClubMemberList(widget.clubName);
                    // Navigator.push(
                    //     context,
                    //     MaterialPageRoute(
